...
var helpers = exports.helpers = {
	...
	processAmazonOrder: function(orderInfo) {
		if(parseInt(orderInfo['NumberOfItemsUnshipped']) < 1)
			return null;
		console.log(orderInfo['OrderTotal']);
		order ={};
		buyer = {};
		orderItems = [];
		order['relative_order_id'] = orderInfo['AmazonOrderId'];
		order['amount'] = parseFloat(orderInfo['OrderTotal'][0]['Amount']);
		order['currencyType'] = orderInfo['OrderTotal'][0]['CurrencyCode'];
		order['orderDate'] = unescape(orderInfo.PurchaseDate);
		order['serviceType'] = 'amazon';
		...
		buyer['email'] = orderInfo.BuyerEmail;
		buyer['amazonName'] = orderInfo.BuyerName;
		buyer['shippingAddress'] = new Object();
		buyer['shippingAddress']['Name'] = orderInfo.Name;
		buyer['shippingAddress']['Street1'] = orderInfo.AddressLine1;
		buyer['shippingAddress']['Street2'] = orderInfo.AddressLine2;
		buyer['shippingAddress']['CityName'] = orderInfo.City;
		buyer['shippingAddress']['StateOrProvince'] = orderInfo.StateOrRegion;
		buyer['shippingAddress']['PostalCode'] = orderInfo.PostalCode;
		buyer['shippingAddress']['Country'] = orderInfo.Country;
